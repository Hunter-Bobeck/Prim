...
// for the first node only (requires the nodes container to be empty): spawns a node at this controller's position whenever the touchpad is pressing, so long as the controller is not currently colliding with a node //
public class NodeSpawner : MonoBehaviour
{
    ...
    void Update()
    {
        if (nodesContainer.childCount == 0)     // if there are no nodes currently
        {
